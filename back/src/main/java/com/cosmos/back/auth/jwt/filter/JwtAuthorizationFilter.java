...
public class JwtAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        Long userSeq = jwtService.validAccessToken(token);
...
//        }
        ...
        /**
         * access토큰 만료 > 바로 access, refresh 재발급
         * 이게 되나..?
         */
        if(userSeq == null) {
            System.out.println("access 토큰 만료");
            JwtToken newJwtToken = jwtService.validRefreshToken(userSeq);
        }
        User user = userRepository.findByUserSeq(userSeq); //정상적인 access 토큰 사용자
