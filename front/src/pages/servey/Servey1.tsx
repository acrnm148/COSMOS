...
export default function Servey1(){
    ...
    return(
        <div className="">
            ...
            <h1 className="font-baloo text-base text-white mb-2">1. 가장 중요한 식당 포인트는?</h1>
            <ul className="flex flex-wrap w-full grid-rows-3 gap-2 justify-center">
            {['인스타 핫플', '컨셉이 있는 식당', '높은 평점', '무한리필 뷔페', '집밥 느낌의 한식', ,'평소 못 가본 오마카세', '이색 맛집', '맛있게 먹었던 식당'].map((item, key) => {
                return (
                <div
                    key={key}
                    onClick={()=>submit1(key)}
                    className={'text-sm flex justify-center w-24 sm:w-36 p-4 content-center text-center bg-darkMain4 pt-10 pb-10 rounded-lg hover:bg-opacity-75 border-solid border-2 border-darkMain4'
                }
                >
