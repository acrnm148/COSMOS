...
export function ScheduleDetail() {
  ...
  const { data } = useQuery({
    ...
    queryFn: () =>
      getPlaceCoupleReview(
        loginUser.seq,
        Number(loginUser.coupleId),
        place.placeId,
        10,
        offset
      ),
    ...
    if (data) {
      ...
      data.map(
        (review: {
          ...
          images: any;
          contents: any;
          categories: { reviewCategoryCode: string }[];
          indiReviewCategories: { reviewCategory: any }[];
          contentOpen: any;
          imageOpen: any;
          createdTime: any;
        }) => {
          if (String(review.userId) === String(loginUser.seq)) {
            setUserReview({
              score: review.score,
              photos: review.images.map(
                (img: { imageUrl: any }) => img.imageUrl
              ),
  ...
  return (
    <div className="bg-lightMain2 mb-5">
      ...
      <div
        className={
          "bg-white rounded-t-lg w-full flex flex-col items-between" +
          (!showReview ? "p-2" : "w-full")
        }
      >
        <div>
          ...
          {!showReview && (
            <div>
              <div
                className={`detailImg h-full w-full rounded-lg max-h-[50vh] overflow-hidden flex items-center`}
              >
            ...
            <div className="">
              <div className="누구리뷰 w-full flex h-10">
                <div
                  className={
                    showMine
                      ? "bg-lightMain w-[50%] flex justify-center rounded-t-lg items-center"
                ...
                <div
                  className={
                    showMine
                      ? " w-[50%] flex justify-center rounded-t-lg items-center"
                      : "bg-lightMain w-[50%] flex justify-center items-center"
                  }
                >
                  너의리뷰
                </div>
  ...
  return (
    <div className="mb-10">
      ...
      <div className="flex mt-1 mb-1">
        <p className="bg-lightMain2 px-2 py-1 rounded-xl">
          {props.Review.cateQ}
        </p>
        <p className="bg-lightMain2 px-2 py-1 rounded-xl">
          {props.Review.commonQ}
        </p>
      </div>
      <div className="flex">
        {props.Review.photos?.map((photo: string | undefined) => {
          return <img className="h-20 w-20 rounded-lg" src={photo}></img>;
        })}
      </div>
      ...
      {props.isMine && (
        <div className="h-20 w-full cursor-pointer w-full flex flex-col justify-end items-center p-4">
          <div onClick={() => props.setShowReview(true)} className="w-full">
            <ReviewSet edit={true} />
          </div>
        </div>
      )}
